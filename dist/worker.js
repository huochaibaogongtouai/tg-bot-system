var N=!1,it=["CREATE TABLE IF NOT EXISTS system_config (key TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at TEXT DEFAULT (datetime('now')))","CREATE TABLE IF NOT EXISTS sessions (session_id TEXT PRIMARY KEY, created_at TEXT DEFAULT (datetime('now')), expires_at TEXT NOT NULL)","CREATE TABLE IF NOT EXISTS bots (id INTEGER PRIMARY KEY AUTOINCREMENT, bot_id TEXT UNIQUE NOT NULL, name TEXT NOT NULL, token TEXT NOT NULL, status TEXT DEFAULT 'active', webhook_url TEXT, admin_chat_id TEXT, created_at TEXT DEFAULT (datetime('now')), updated_at TEXT DEFAULT (datetime('now')))","CREATE TABLE IF NOT EXISTS forward_rules (id INTEGER PRIMARY KEY AUTOINCREMENT, bot_id TEXT NOT NULL, source_channel TEXT NOT NULL, target_chats TEXT NOT NULL, delay_seconds INTEGER DEFAULT 5, skip_keywords TEXT DEFAULT '', enabled INTEGER DEFAULT 1, created_at TEXT DEFAULT (datetime('now')), updated_at TEXT DEFAULT (datetime('now')))","CREATE TABLE IF NOT EXISTS auto_replies (id INTEGER PRIMARY KEY AUTOINCREMENT, bot_id TEXT NOT NULL, keyword TEXT NOT NULL, reply_content TEXT NOT NULL, match_mode TEXT DEFAULT 'fuzzy', enabled INTEGER DEFAULT 1, created_at TEXT DEFAULT (datetime('now')))","CREATE TABLE IF NOT EXISTS group_configs (id INTEGER PRIMARY KEY AUTOINCREMENT, bot_id TEXT NOT NULL, group_id TEXT NOT NULL, welcome_message TEXT DEFAULT '', banned_words TEXT DEFAULT '', enabled INTEGER DEFAULT 1, created_at TEXT DEFAULT (datetime('now')), updated_at TEXT DEFAULT (datetime('now')), UNIQUE(bot_id, group_id))","CREATE TABLE IF NOT EXISTS message_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, bot_id TEXT NOT NULL, chat_id TEXT NOT NULL, chat_type TEXT, user_id TEXT, username TEXT, first_name TEXT, message_text TEXT, message_type TEXT DEFAULT 'text', raw_data TEXT, created_at TEXT DEFAULT (datetime('now')))","CREATE TABLE IF NOT EXISTS member_events (id INTEGER PRIMARY KEY AUTOINCREMENT, bot_id TEXT NOT NULL, group_id TEXT NOT NULL, user_id TEXT NOT NULL, username TEXT, first_name TEXT, event_type TEXT NOT NULL, created_at TEXT DEFAULT (datetime('now')))","CREATE TABLE IF NOT EXISTS scheduled_tasks (id INTEGER PRIMARY KEY AUTOINCREMENT, bot_id TEXT NOT NULL, task_type TEXT NOT NULL, target_chat TEXT NOT NULL, content TEXT NOT NULL, repeat_mode TEXT DEFAULT 'once', next_run_at TEXT NOT NULL, last_run_at TEXT, enabled INTEGER DEFAULT 1, created_at TEXT DEFAULT (datetime('now')))","CREATE TABLE IF NOT EXISTS customer_messages (id INTEGER PRIMARY KEY AUTOINCREMENT, bot_id TEXT NOT NULL, original_user_id TEXT NOT NULL, original_chat_id TEXT NOT NULL, forwarded_message_id TEXT, admin_chat_id TEXT, created_at TEXT DEFAULT (datetime('now')))","CREATE TABLE IF NOT EXISTS forward_queue (id INTEGER PRIMARY KEY AUTOINCREMENT, bot_id TEXT NOT NULL, rule_id INTEGER NOT NULL, source_chat_id TEXT NOT NULL, source_message_id TEXT NOT NULL, target_chat TEXT NOT NULL, scheduled_at TEXT NOT NULL, status TEXT DEFAULT 'pending', created_at TEXT DEFAULT (datetime('now')))","CREATE INDEX IF NOT EXISTS idx_message_logs_bot ON message_logs(bot_id)","CREATE INDEX IF NOT EXISTS idx_message_logs_chat ON message_logs(chat_id)","CREATE INDEX IF NOT EXISTS idx_message_logs_created ON message_logs(created_at)","CREATE INDEX IF NOT EXISTS idx_member_events_bot ON member_events(bot_id)","CREATE INDEX IF NOT EXISTS idx_forward_rules_bot ON forward_rules(bot_id)","CREATE INDEX IF NOT EXISTS idx_auto_replies_bot ON auto_replies(bot_id)","CREATE INDEX IF NOT EXISTS idx_scheduled_tasks_next ON scheduled_tasks(next_run_at)","CREATE INDEX IF NOT EXISTS idx_forward_queue_scheduled ON forward_queue(scheduled_at, status)","CREATE INDEX IF NOT EXISTS idx_customer_messages_bot ON customer_messages(bot_id, forwarded_message_id)"];function dt(t=12){let n="ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz23456789",a="",e=new Uint8Array(t);crypto.getRandomValues(e);for(let r=0;r<t;r++)a+=n[e[r]%n.length];return a}async function D(t){if(N)return;try{if(await t.prepare("SELECT value FROM system_config WHERE key = 'admin_password'").first()){N=!0;return}}catch{}for(let a of it)try{await t.prepare(a).run()}catch{}if(!await t.prepare("SELECT value FROM system_config WHERE key = 'admin_password'").first()){let a=dt();await t.prepare("INSERT INTO system_config (key, value) VALUES ('admin_password', ?)").bind(a).run(),console.log(`[INIT] Default admin password generated: ${a}`)}N=!0}async function T(t,n){let a=await t.prepare("SELECT value FROM system_config WHERE key = ?").bind(n).first();return a?a.value:null}async function x(t,n,a){await t.prepare("INSERT INTO system_config (key, value, updated_at) VALUES (?, ?, datetime('now')) ON CONFLICT(key) DO UPDATE SET value = ?, updated_at = datetime('now')").bind(n,a,a).run()}function ct(){return crypto.randomUUID()+"-"+Date.now().toString(36)}async function H(t){let n=ct(),a=new Date(Date.now()+1440*60*1e3).toISOString();return await t.prepare("INSERT INTO sessions (session_id, expires_at) VALUES (?, ?)").bind(n,a).run(),n}async function lt(t,n){return n?!!await t.prepare("SELECT * FROM sessions WHERE session_id = ? AND expires_at > datetime('now')").bind(n).first():!1}async function X(t,n){n&&await t.prepare("DELETE FROM sessions WHERE session_id = ?").bind(n).run()}function M(t){let a=(t.headers.get("Cookie")||"").match(/session_id=([^;]+)/);return a?a[1]:null}function W(t){return`session_id=${t}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=86400`}function z(){return"session_id=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0"}async function G(t,n){return await T(t,"admin_password")===n}async function j(t,n){let a=M(n);return await lt(t,a)}var pt="https://api.telegram.org/bot";async function f(t,n,a={}){let e=`${pt}${t}/${n}`;try{let i=await(await fetch(e,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})).json();return i.ok||console.error(`[TG API Error] ${n}:`,i.description),i}catch(r){return console.error(`[TG API Exception] ${n}:`,r.message),{ok:!1,description:r.message}}}async function g(t,n,a,e={}){return f(t,"sendMessage",{chat_id:n,text:a,parse_mode:"HTML",...e})}async function Y(t,n,a,e){return f(t,"forwardMessage",{chat_id:n,from_chat_id:a,message_id:e})}async function I(t,n,a,e){return f(t,"copyMessage",{chat_id:n,from_chat_id:a,message_id:e})}async function V(t,n,a){return f(t,"deleteMessage",{chat_id:n,message_id:a})}async function ut(t,n,a,e={}){return f(t,"sendPhoto",{chat_id:n,photo:a,...e})}async function mt(t,n,a,e={}){return f(t,"sendVideo",{chat_id:n,video:a,...e})}async function bt(t,n,a,e={}){return f(t,"sendDocument",{chat_id:n,document:a,...e})}async function gt(t,n,a,e={}){return f(t,"sendAudio",{chat_id:n,audio:a,...e})}async function ft(t,n,a,e={}){return f(t,"sendVoice",{chat_id:n,voice:a,...e})}async function ht(t,n,a,e={}){return f(t,"sendSticker",{chat_id:n,sticker:a,...e})}async function Et(t,n,a,e={}){return f(t,"sendAnimation",{chat_id:n,animation:a,...e})}async function K(t,n,a){if(a.photo&&a.photo.length>0){let e=a.photo[a.photo.length-1];return ut(t,n,e.file_id,{caption:a.caption||"",parse_mode:"HTML"})}return a.video?mt(t,n,a.video.file_id,{caption:a.caption||"",parse_mode:"HTML"}):a.document?bt(t,n,a.document.file_id,{caption:a.caption||"",parse_mode:"HTML"}):a.audio?gt(t,n,a.audio.file_id,{caption:a.caption||"",parse_mode:"HTML"}):a.voice?ft(t,n,a.voice.file_id):a.sticker?ht(t,n,a.sticker.file_id):a.animation?Et(t,n,a.animation.file_id,{caption:a.caption||"",parse_mode:"HTML"}):a.text?g(t,n,a.text):null}async function J(t,n){return f(t,"setWebhook",{url:n,allowed_updates:["message","channel_post","chat_member","my_chat_member"]})}async function Q(t){return f(t,"deleteWebhook",{drop_pending_updates:!1})}async function q(t){return f(t,"getMe")}async function L(t,n){return f(t,"setMyCommands",{commands:n})}function S(t){return t.text||t.caption||""}function et(t){return t.text?"text":t.photo?"photo":t.video?"video":t.document?"document":t.audio?"audio":t.voice?"voice":t.sticker?"sticker":t.animation?"animation":t.poll?"poll":t.location?"location":t.contact?"contact":t.video_note?"video_note":"other"}async function at(t,n,a){let{bot_id:e,token:r,admin_chat_id:i}=n;try{if(a.channel_post){await vt(t,e,r,a.channel_post);return}if(a.message){let c=a.message;if(c.new_chat_members){await _t(t,e,r,c);return}if(c.left_chat_member){await wt(t,e,r,c);return}if(await Tt(t,e,c),i&&c.reply_to_message&&String(c.chat.id)===String(i)){await It(t,e,r,c);return}if((c.chat.type==="group"||c.chat.type==="supergroup")&&await yt(t,e,r,c))return;if(c.text&&c.text.startsWith("/")){await xt(t,e,r,c),c.text.startsWith("/start")&&c.chat.type==="private"&&await Z(t,e,r,c),c.chat.type==="private"&&i&&String(c.from.id)!==String(i)&&await tt(t,e,r,c,i);return}await kt(t,e,r,c),c.chat.type==="private"&&i&&String(c.from.id)!==String(i)&&(await Z(t,e,r,c),await tt(t,e,r,c,i))}}catch(c){console.error(`[Webhook Error] bot=${e}:`,c.message)}}async function vt(t,n,a,e){let r=String(e.chat.id),i=S(e);await t.prepare("INSERT INTO message_logs (bot_id, chat_id, chat_type, user_id, username, message_text, message_type, created_at) VALUES (?, ?, 'channel', '', ?, ?, ?, datetime('now'))").bind(n,r,e.chat.title||"",i,et(e)).run();let c=await t.prepare("SELECT * FROM forward_rules WHERE bot_id = ? AND enabled = 1 AND source_channel = ?").bind(n,r).all();if(!(!c.results||c.results.length===0))for(let m of c.results){if(m.skip_keywords&&m.skip_keywords.split(",").map(w=>w.trim()).filter(w=>w).some(w=>i.includes(w))){console.log(`[Forward] Skipped message due to keyword filter, rule=${m.id}`);continue}let b=m.target_chats.split(",").map(_=>_.trim()).filter(_=>_),u=m.delay_seconds||0,v=new Date(Date.now()+u*1e3).toISOString();for(let _ of b)await t.prepare("INSERT INTO forward_queue (bot_id, rule_id, source_chat_id, source_message_id, target_chat, scheduled_at, status) VALUES (?, ?, ?, ?, ?, ?, 'pending')").bind(n,m.id,r,String(e.message_id),_,v).run()}}async function _t(t,n,a,e){let r=String(e.chat.id);for(let i of e.new_chat_members){await t.prepare("INSERT INTO member_events (bot_id, group_id, user_id, username, first_name, event_type, created_at) VALUES (?, ?, ?, ?, ?, 'join', datetime('now'))").bind(n,r,String(i.id),i.username||"",i.first_name||"").run();let c=await t.prepare("SELECT * FROM group_configs WHERE bot_id = ? AND group_id = ? AND enabled = 1").bind(n,r).first();if(c&&c.welcome_message){let m=c.welcome_message.replace(/{name}/g,i.first_name||i.username||"\u65B0\u6210\u5458").replace(/{username}/g,i.username?`@${i.username}`:"").replace(/{group}/g,e.chat.title||"");await g(a,r,m)}}}async function wt(t,n,a,e){let r=String(e.chat.id),i=e.left_chat_member;await t.prepare("INSERT INTO member_events (bot_id, group_id, user_id, username, first_name, event_type, created_at) VALUES (?, ?, ?, ?, ?, 'leave', datetime('now'))").bind(n,r,String(i.id),i.username||"",i.first_name||"").run()}async function Tt(t,n,a){let e=String(a.chat.id),r=a.chat.type,i=a.from?String(a.from.id):"",c=a.from&&a.from.username||"",m=a.from&&a.from.first_name||"",b=S(a),u=et(a);await t.prepare("INSERT INTO message_logs (bot_id, chat_id, chat_type, user_id, username, first_name, message_text, message_type, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))").bind(n,e,r,i,c,m,b,u).run()}async function yt(t,n,a,e){let r=String(e.chat.id),i=S(e).toLowerCase();if(!i)return!1;let c=await t.prepare("SELECT * FROM group_configs WHERE bot_id = ? AND group_id = ? AND enabled = 1").bind(n,r).first();if(!c||!c.banned_words)return!1;if(c.banned_words.split(",").map(u=>u.trim().toLowerCase()).filter(u=>u).find(u=>i.includes(u))){await V(a,r,e.message_id);let u=e.from.first_name||e.from.username||"\u7528\u6237";return await g(a,r,`\u26A0\uFE0F <b>${u}</b> \u7684\u6D88\u606F\u5305\u542B\u8FDD\u7981\u5185\u5BB9\uFF0C\u5DF2\u81EA\u52A8\u62E6\u622A\u3002`),!0}return!1}async function xt(t,n,a,e){let r=e.text.trim(),i=String(e.chat.id),c=r.split(/\s+/);switch(c[0].split("@")[0].toLowerCase()){case"/start":let b=await T(t,`bot_${n}_start_reply`);b?await g(a,i,b):await g(a,i,`\u{1F44B} \u6B22\u8FCE\u4F7F\u7528\u672C\u673A\u5668\u4EBA\uFF01

\u53D1\u9001\u4EFB\u610F\u6D88\u606F\u5373\u53EF\u4E0E\u7BA1\u7406\u5458\u6C9F\u901A\u3002`);break;case"/help":let u=await T(t,`bot_${n}_help_reply`);u?await g(a,i,u):await g(a,i,`\u{1F4D6} <b>\u53EF\u7528\u6307\u4EE4</b>
...
\u4F1A\u8BDDID: <code>${i}</code>
\u7528\u6237ID: <code>${v}</code>
\u7C7B\u578B: ${e.chat.type}`);break;case"/remind":await Rt(t,n,a,e,c);break;default:break}}async function Rt(t,n,a,e,r){let i=String(e.chat.id);if(r.length<3){await g(a,i,`\u26A0\uFE0F \u7528\u6CD5: /remind [\u5206\u949F\u6570] [\u63D0\u9192\u5185\u5BB9]
\u4F8B\u5982: /remind 30 \u5F00\u4F1A`);return}let c=parseInt(r[1]);if(isNaN(c)||c<1||c>10080){await g(a,i,"\u26A0\uFE0F \u5206\u949F\u6570\u5FC5\u987B\u5728 1-10080 \u4E4B\u95F4");return}let m=r.slice(2).join(" "),b=new Date(Date.now()+c*60*1e3).toISOString();await t.prepare("INSERT INTO scheduled_tasks (bot_id, task_type, target_chat, content, repeat_mode, next_run_at, enabled) VALUES (?, 'remind', ?, ?, 'once', ?, 1)").bind(n,i,m,b).run(),await g(a,i,`\u2705 \u63D0\u9192\u5DF2\u8BBE\u7F6E\uFF01\u5C06\u5728 ${c} \u5206\u949F\u540E\u63D0\u9192\u4F60\uFF1A
${m}`)}async function kt(t,n,a,e){let r=S(e);if(!r)return;let i=String(e.chat.id),c=await t.prepare("SELECT * FROM auto_replies WHERE bot_id = ? AND enabled = 1").bind(n).all();if(!(!c.results||c.results.length===0))for(let m of c.results){let b=!1;if(m.match_mode==="exact"?b=r.trim()===m.keyword.trim():b=r.toLowerCase().includes(m.keyword.toLowerCase()),b){await g(a,i,m.reply_content,{reply_to_message_id:e.message_id});break}}}async function Z(t,n,a,e){let r=String(e.from.id),i=String(e.chat.id),c=await T(t,`bot_${n}_first_contact_reply`);if(!c)return;if(!await t.prepare("SELECT id FROM customer_messages WHERE bot_id = ? AND original_user_id = ? LIMIT 1").bind(n,r).first()){let b=e.from.first_name||e.from.username||"\u7528\u6237",u=c.replace(/{name}/g,b).replace(/{username}/g,e.from.username?`@${e.from.username}`:"");await g(a,i,u)}}async function tt(t,n,a,e,r){let i=await Y(a,r,e.chat.id,e.message_id);i.ok&&await t.prepare("INSERT INTO customer_messages (bot_id, original_user_id, original_chat_id, forwarded_message_id, admin_chat_id) VALUES (?, ?, ?, ?, ?)").bind(n,String(e.from.id),String(e.chat.id),String(i.result.message_id),r).run()}async function It(t,n,a,e){let r=e.reply_to_message;if(!r)return;let i=await t.prepare("SELECT * FROM customer_messages WHERE bot_id = ? AND forwarded_message_id = ? ORDER BY created_at DESC LIMIT 1").bind(n,String(r.message_id)).first();i&&(await K(a,i.original_user_id,e)||await I(a,i.original_user_id,e.chat.id,e.message_id))}function p(t,n=200){return new Response(JSON.stringify(t),{status:n,headers:{"Content-Type":"application/json"}})}function ot(t,n){return new Response(t,{headers:{"Content-Type":"text/csv; charset=utf-8","Content-Disposition":`attachment; filename="${n}"`}})}async function nt(t,n,a){let e=a.pathname,r=n.method;try{if(e==="/api/config/password"&&r==="PUT"){let s=await n.json(),o=await T(t,"admin_password");return s.old_password!==o?p({error:"\u5F53\u524D\u5BC6\u7801\u9519\u8BEF"},400):!s.new_password||s.new_password.length<6?p({error:"\u65B0\u5BC6\u7801\u957F\u5EA6\u4E0D\u80FD\u5C11\u4E8E6\u4F4D"},400):(await x(t,"admin_password",s.new_password),p({success:!0,message:"\u5BC6\u7801\u4FEE\u6539\u6210\u529F"}))}if(e==="/api/config/default-password"&&r==="GET"){let s=await T(t,"admin_password");return p({password:s})}if(e==="/api/bots"&&r==="GET"){let s=await t.prepare("SELECT id, bot_id, name, token, status, webhook_url, admin_chat_id, created_at FROM bots ORDER BY created_at DESC").all();return p({bots:s.results||[]})}if(e==="/api/bots"&&r==="POST"){let s=await n.json();if(!s.name||!s.token)return p({error:"\u540D\u79F0\u548CToken\u4E0D\u80FD\u4E3A\u7A7A"},400);let o=await q(s.token);if(!o.ok)return p({error:"Bot Token \u65E0\u6548\uFF0C\u8BF7\u68C0\u67E5\u540E\u91CD\u8BD5"},400);let l=String(o.result.id);if(await t.prepare("SELECT * FROM bots WHERE bot_id = ?").bind(l).first())return p({error:"\u8BE5\u673A\u5668\u4EBA\u5DF2\u5B58\u5728"},400);await t.prepare("INSERT INTO bots (bot_id, name, token, admin_chat_id) VALUES (?, ?, ?, ?)").bind(l,s.name,s.token,s.admin_chat_id||"").run();let E=`${a.origin}/webhook/${l}`,y=await J(s.token,E);return y.ok&&await t.prepare("UPDATE bots SET webhook_url = ? WHERE bot_id = ?").bind(E,l).run(),await L(s.token,[{command:"start",description:"\u5F00\u59CB\u4F7F\u7528 / \u542F\u52A8\u673A\u5668\u4EBA"},{command:"help",description:"\u5E2E\u52A9\u4FE1\u606F / \u529F\u80FD\u8BF4\u660E"},{command:"id",description:"\u83B7\u53D6\u5F53\u524D\u4F1A\u8BDDID"}]),p({success:!0,bot_id:l,bot_username:o.result.username,webhook_url:E,webhook_set:y.ok})}let i=e.match(/^\/api\/bots\/([^/]+)$/);if(i){let s=i[1];if(r==="GET"){let o=await t.prepare("SELECT * FROM bots WHERE bot_id = ?").bind(s).first();if(!o)return p({error:"\u673A\u5668\u4EBA\u4E0D\u5B58\u5728"},404);let l=await T(t,`bot_${s}_first_contact_reply`)||"",d=await T(t,`bot_${s}_start_reply`)||"",h=await T(t,`bot_${s}_help_reply`)||"";return p({bot:o,first_contact_reply:l,start_reply:d,help_reply:h})}if(r==="PUT"){let o=await n.json(),l=[],d=[];if(o.name!==void 0&&(l.push("name = ?"),d.push(o.name)),o.admin_chat_id!==void 0&&(l.push("admin_chat_id = ?"),d.push(o.admin_chat_id)),o.status!==void 0&&(l.push("status = ?"),d.push(o.status)),l.length>0&&(l.push("updated_at = datetime('now')"),d.push(s),await t.prepare(`UPDATE bots SET ${l.join(", ")} WHERE bot_id = ?`).bind(...d).run()),o.first_contact_reply!==void 0&&await x(t,`bot_${s}_first_contact_reply`,o.first_contact_reply),o.start_reply!==void 0&&await x(t,`bot_${s}_start_reply`,o.start_reply),o.help_reply!==void 0&&await x(t,`bot_${s}_help_reply`,o.help_reply),o.bot_commands!==void 0){let h=await t.prepare("SELECT token FROM bots WHERE bot_id = ?").bind(s).first();if(h)try{let E=JSON.parse(o.bot_commands);Array.isArray(E)&&E.length>0&&(await L(h.token,E),await x(t,`bot_${s}_commands`,o.bot_commands))}catch{}}return p({success:!0})}if(r==="DELETE"){let o=await t.prepare("SELECT * FROM bots WHERE bot_id = ?").bind(s).first();return o&&(await Q(o.token),await t.prepare("DELETE FROM forward_rules WHERE bot_id = ?").bind(s).run(),await t.prepare("DELETE FROM auto_replies WHERE bot_id = ?").bind(s).run(),await t.prepare("DELETE FROM group_configs WHERE bot_id = ?").bind(s).run(),await t.prepare("DELETE FROM message_logs WHERE bot_id = ?").bind(s).run(),await t.prepare("DELETE FROM member_events WHERE bot_id = ?").bind(s).run(),await t.prepare("DELETE FROM scheduled_tasks WHERE bot_id = ?").bind(s).run(),await t.prepare("DELETE FROM customer_messages WHERE bot_id = ?").bind(s).run(),await t.prepare("DELETE FROM forward_queue WHERE bot_id = ?").bind(s).run(),await t.prepare("DELETE FROM system_config WHERE key LIKE ?").bind(`bot_${s}_%`).run(),await t.prepare("DELETE FROM bots WHERE bot_id = ?").bind(s).run()),p({success:!0})}}let c=e.match(/^\/api\/bots\/([^/]+)\/commands$/);if(c&&r==="PUT"){let s=c[1],o=await n.json(),l=await t.prepare("SELECT token FROM bots WHERE bot_id = ?").bind(s).first();if(!l)return p({error:"\u673A\u5668\u4EBA\u4E0D\u5B58\u5728"},404);let d=o.commands||[],h=await L(l.token,d);return h.ok?(await x(t,`bot_${s}_commands`,JSON.stringify(d)),p({success:!0,message:"\u547D\u4EE4\u83DC\u5355\u5DF2\u66F4\u65B0"})):p({error:"\u66F4\u65B0\u547D\u4EE4\u83DC\u5355\u5931\u8D25: "+(h.description||"")},400)}let m=e.match(/^\/api\/bots\/([^/]+)\/forward-rules$/);if(m){let s=m[1];if(r==="GET"){let o=await t.prepare("SELECT * FROM forward_rules WHERE bot_id = ? ORDER BY created_at DESC").bind(s).all();return p({rules:o.results||[]})}if(r==="POST"){let o=await n.json();return!o.source_channel||!o.target_chats?p({error:"\u6E90\u9891\u9053\u548C\u76EE\u6807\u7FA4\u4E0D\u80FD\u4E3A\u7A7A"},400):(await t.prepare("INSERT INTO forward_rules (bot_id, source_channel, target_chats, delay_seconds, skip_keywords, enabled) VALUES (?, ?, ?, ?, ?, ?)").bind(s,o.source_channel,o.target_chats,o.delay_seconds||5,o.skip_keywords||"",o.enabled!==void 0?o.enabled:1).run(),p({success:!0}))}}let b=e.match(/^\/api\/forward-rules\/(\d+)$/);if(b){let s=parseInt(b[1]);if(r==="PUT"){let o=await n.json(),l=[],d=[];return o.source_channel!==void 0&&(l.push("source_channel = ?"),d.push(o.source_channel)),o.target_chats!==void 0&&(l.push("target_chats = ?"),d.push(o.target_chats)),o.delay_seconds!==void 0&&(l.push("delay_seconds = ?"),d.push(o.delay_seconds)),o.skip_keywords!==void 0&&(l.push("skip_keywords = ?"),d.push(o.skip_keywords)),o.enabled!==void 0&&(l.push("enabled = ?"),d.push(o.enabled)),l.length>0&&(l.push("updated_at = datetime('now')"),d.push(s),await t.prepare(`UPDATE forward_rules SET ${l.join(", ")} WHERE id = ?`).bind(...d).run()),p({success:!0})}if(r==="DELETE")return await t.prepare("DELETE FROM forward_rules WHERE id = ?").bind(s).run(),p({success:!0})}let u=e.match(/^\/api\/bots\/([^/]+)\/auto-replies$/);if(u){let s=u[1];if(r==="GET"){let o=await t.prepare("SELECT * FROM auto_replies WHERE bot_id = ? ORDER BY created_at DESC").bind(s).all();return p({replies:o.results||[]})}if(r==="POST"){let o=await n.json();return!o.keyword||!o.reply_content?p({error:"\u5173\u952E\u8BCD\u548C\u56DE\u590D\u5185\u5BB9\u4E0D\u80FD\u4E3A\u7A7A"},400):(await t.prepare("INSERT INTO auto_replies (bot_id, keyword, reply_content, match_mode, enabled) VALUES (?, ?, ?, ?, ?)").bind(s,o.keyword,o.reply_content,o.match_mode||"fuzzy",o.enabled!==void 0?o.enabled:1).run(),p({success:!0}))}}let v=e.match(/^\/api\/auto-replies\/(\d+)$/);if(v){let s=parseInt(v[1]);if(r==="PUT"){let o=await n.json(),l=[],d=[];return o.keyword!==void 0&&(l.push("keyword = ?"),d.push(o.keyword)),o.reply_content!==void 0&&(l.push("reply_content = ?"),d.push(o.reply_content)),o.match_mode!==void 0&&(l.push("match_mode = ?"),d.push(o.match_mode)),o.enabled!==void 0&&(l.push("enabled = ?"),d.push(o.enabled)),l.length>0&&(d.push(s),await t.prepare(`UPDATE auto_replies SET ${l.join(", ")} WHERE id = ?`).bind(...d).run()),p({success:!0})}if(r==="DELETE")return await t.prepare("DELETE FROM auto_replies WHERE id = ?").bind(s).run(),p({success:!0})}let _=e.match(/^\/api\/bots\/([^/]+)\/group-configs$/);if(_){let s=_[1];if(r==="GET"){let o=await t.prepare("SELECT * FROM group_configs WHERE bot_id = ? ORDER BY created_at DESC").bind(s).all();return p({configs:o.results||[]})}if(r==="POST"){let o=await n.json();return o.group_id?(await t.prepare("INSERT INTO group_configs (bot_id, group_id, welcome_message, banned_words, enabled) VALUES (?, ?, ?, ?, ?) ON CONFLICT(bot_id, group_id) DO UPDATE SET welcome_message = ?, banned_words = ?, enabled = ?, updated_at = datetime('now')").bind(s,o.group_id,o.welcome_message||"",o.banned_words||"",o.enabled!==void 0?o.enabled:1,o.welcome_message||"",o.banned_words||"",o.enabled!==void 0?o.enabled:1).run(),p({success:!0})):p({error:"\u7FA4ID\u4E0D\u80FD\u4E3A\u7A7A"},400)}}let R=e.match(/^\/api\/group-configs\/(\d+)$/);if(R){let s=parseInt(R[1]);if(r==="PUT"){let o=await n.json(),l=[],d=[];return o.welcome_message!==void 0&&(l.push("welcome_message = ?"),d.push(o.welcome_message)),o.banned_words!==void 0&&(l.push("banned_words = ?"),d.push(o.banned_words)),o.enabled!==void 0&&(l.push("enabled = ?"),d.push(o.enabled)),l.length>0&&(l.push("updated_at = datetime('now')"),d.push(s),await t.prepare(`UPDATE group_configs SET ${l.join(", ")} WHERE id = ?`).bind(...d).run()),p({success:!0})}if(r==="DELETE")return await t.prepare("DELETE FROM group_configs WHERE id = ?").bind(s).run(),p({success:!0})}let w=e.match(/^\/api\/bots\/([^/]+)\/tasks$/);if(w){let s=w[1];if(r==="GET"){let o=await t.prepare("SELECT * FROM scheduled_tasks WHERE bot_id = ? ORDER BY created_at DESC").bind(s).all();return p({tasks:o.results||[]})}if(r==="POST"){let o=await n.json();return!o.target_chat||!o.content?p({error:"\u76EE\u6807\u548C\u5185\u5BB9\u4E0D\u80FD\u4E3A\u7A7A"},400):o.next_run_at?(await t.prepare("INSERT INTO scheduled_tasks (bot_id, task_type, target_chat, content, repeat_mode, next_run_at, enabled) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(s,o.task_type||"send",o.target_chat,o.content,o.repeat_mode||"once",o.next_run_at,o.enabled!==void 0?o.enabled:1).run(),p({success:!0})):p({error:"\u6267\u884C\u65F6\u95F4\u4E0D\u80FD\u4E3A\u7A7A"},400)}}let k=e.match(/^\/api\/tasks\/(\d+)$/);if(k){let s=parseInt(k[1]);if(r==="PUT"){let o=await n.json(),l=[],d=[];return o.target_chat!==void 0&&(l.push("target_chat = ?"),d.push(o.target_chat)),o.content!==void 0&&(l.push("content = ?"),d.push(o.content)),o.repeat_mode!==void 0&&(l.push("repeat_mode = ?"),d.push(o.repeat_mode)),o.next_run_at!==void 0&&(l.push("next_run_at = ?"),d.push(o.next_run_at)),o.enabled!==void 0&&(l.push("enabled = ?"),d.push(o.enabled)),l.length>0&&(d.push(s),await t.prepare(`UPDATE scheduled_tasks SET ${l.join(", ")} WHERE id = ?`).bind(...d).run()),p({success:!0})}if(r==="DELETE")return await t.prepare("DELETE FROM scheduled_tasks WHERE id = ?").bind(s).run(),p({success:!0})}let O=e.match(/^\/api\/bots\/([^/]+)\/logs$/);if(O){let s=O[1],o=a.searchParams,l=parseInt(o.get("page")||"1"),d=Math.min(parseInt(o.get("limit")||"50"),200),h=(l-1)*d,E=await t.prepare("SELECT COUNT(*) as total FROM message_logs WHERE bot_id = ?").bind(s).first(),y=await t.prepare("SELECT * FROM message_logs WHERE bot_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(s,d,h).all();return p({logs:y.results||[],total:E.total,page:l,limit:d})}let A=e.match(/^\/api\/bots\/([^/]+)\/logs\/export$/);if(A){let s=A[1],o=await t.prepare("SELECT * FROM message_logs WHERE bot_id = ? ORDER BY created_at DESC").bind(s).all(),l=`\uFEFFID,\u673A\u5668\u4EBAID,\u4F1A\u8BDDID,\u4F1A\u8BDD\u7C7B\u578B,\u7528\u6237ID,\u7528\u6237\u540D,\u59D3\u540D,\u6D88\u606F\u5185\u5BB9,\u6D88\u606F\u7C7B\u578B,\u65F6\u95F4
...
`;return ot(l,`messages_${s}_${Date.now()}.csv`)}let B=e.match(/^\/api\/bots\/([^/]+)\/events$/);if(B){let s=B[1],o=a.searchParams,l=parseInt(o.get("page")||"1"),d=Math.min(parseInt(o.get("limit")||"50"),200),h=(l-1)*d,E=await t.prepare("SELECT COUNT(*) as total FROM member_events WHERE bot_id = ?").bind(s).first(),y=await t.prepare("SELECT * FROM member_events WHERE bot_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(s,d,h).all();return p({events:y.results||[],total:E.total,page:l,limit:d})}let U=e.match(/^\/api\/bots\/([^/]+)\/events\/export$/);if(U){let s=U[1],o=await t.prepare("SELECT * FROM member_events WHERE bot_id = ? ORDER BY created_at DESC").bind(s).all(),l=`\uFEFFID,\u673A\u5668\u4EBAID,\u7FA4ID,\u7528\u6237ID,\u7528\u6237\u540D,\u59D3\u540D,\u4E8B\u4EF6\u7C7B\u578B,\u65F6\u95F4
`;for(let d of o.results||[])l+=`${d.id},"${d.bot_id}","${d.group_id}","${d.user_id}","${d.username||""}","${d.first_name||""}","${d.event_type}","${d.created_at}"
`;return ot(l,`events_${s}_${Date.now()}.csv`)}let F=e.match(/^\/api\/bots\/([^/]+)\/logs\/clear$/);if(F&&r==="DELETE"){let s=F[1];return await t.prepare("DELETE FROM message_logs WHERE bot_id = ?").bind(s).run(),p({success:!0})}let $=e.match(/^\/api\/bots\/([^/]+)\/events\/clear$/);if($&&r==="DELETE"){let s=$[1];return await t.prepare("DELETE FROM member_events WHERE bot_id = ?").bind(s).run(),p({success:!0})}let P=e.match(/^\/api\/bots\/([^/]+)\/stats$/);if(P){let s=P[1],o=await t.prepare("SELECT COUNT(*) as c FROM message_logs WHERE bot_id = ?").bind(s).first(),l=await t.prepare("SELECT COUNT(*) as c FROM member_events WHERE bot_id = ? AND event_type = 'join'").bind(s).first(),d=await t.prepare("SELECT COUNT(*) as c FROM member_events WHERE bot_id = ? AND event_type = 'leave'").bind(s).first(),h=await t.prepare("SELECT COUNT(*) as c FROM forward_rules WHERE bot_id = ?").bind(s).first(),E=await t.prepare("SELECT COUNT(*) as c FROM auto_replies WHERE bot_id = ?").bind(s).first(),y=await t.prepare("SELECT COUNT(*) as c FROM scheduled_tasks WHERE bot_id = ? AND enabled = 1").bind(s).first();return p({messages:o.c,joins:l.c,leaves:d.c,forward_rules:h.c,auto_replies:E.c,active_tasks:y.c})}if(e==="/api/stats"&&r==="GET"){let s=await t.prepare("SELECT COUNT(*) as c FROM bots").first(),o=await t.prepare("SELECT COUNT(*) as c FROM message_logs").first(),l=await t.prepare("SELECT COUNT(*) as c FROM scheduled_tasks WHERE enabled = 1").first();return p({bots:s.c,messages:o.c,active_tasks:l.c})}return p({error:"API \u63A5\u53E3\u4E0D\u5B58\u5728"},404)}catch(i){return console.error("[API Error]",i.message),p({error:"\u670D\u52A1\u5668\u5185\u90E8\u9519\u8BEF: "+i.message},500)}}async function rt(t){let n=new Date().toISOString();try{await Lt(t,n),await St(t,n),await t.prepare("DELETE FROM sessions WHERE expires_at <= ?").bind(n).run()}catch(a){console.error("[Cron Error]",a.message)}}async function Lt(t,n){let a=await t.prepare("SELECT fq.*, b.token FROM forward_queue fq JOIN bots b ON fq.bot_id = b.bot_id WHERE fq.status = 'pending' AND fq.scheduled_at <= ? LIMIT 20").bind(n).all();if(!a.results||a.results.length===0)return;for(let r of a.results)try{let i=await I(r.token,r.target_chat,r.source_chat_id,r.source_message_id);i.ok?await t.prepare("UPDATE forward_queue SET status = 'sent' WHERE id = ?").bind(r.id).run():(await t.prepare("UPDATE forward_queue SET status = 'failed' WHERE id = ?").bind(r.id).run(),console.error(`[Forward Queue] Failed to forward message ${r.id}:`,i.description))}catch(i){await t.prepare("UPDATE forward_queue SET status = 'failed' WHERE id = ?").bind(r.id).run(),console.error(`[Forward Queue] Error processing ${r.id}:`,i.message)}let e=new Date(Date.now()-10080*60*1e3).toISOString();await t.prepare("DELETE FROM forward_queue WHERE status IN ('sent', 'failed') AND created_at < ?").bind(e).run()}async function St(t,n){let a=await t.prepare("SELECT st.*, b.token FROM scheduled_tasks st JOIN bots b ON st.bot_id = b.bot_id WHERE st.enabled = 1 AND st.next_run_at <= ? LIMIT 20").bind(n).all();if(!(!a.results||a.results.length===0))for(let e of a.results)try{if(e.task_type==="send"||e.task_type==="remind"){let i=e.content;e.task_type==="remind"&&(i=`\u23F0 <b>\u63D0\u9192</b>

${e.content}`),await g(e.token,e.target_chat,i)}let r=n;if(e.repeat_mode==="once")await t.prepare("UPDATE scheduled_tasks SET enabled = 0, last_run_at = ? WHERE id = ?").bind(r,e.id).run();else if(e.repeat_mode==="hourly"){let i=new Date(new Date(e.next_run_at).getTime()+36e5).toISOString();await t.prepare("UPDATE scheduled_tasks SET next_run_at = ?, last_run_at = ? WHERE id = ?").bind(i,r,e.id).run()}else if(e.repeat_mode==="daily"){let i=new Date(new Date(e.next_run_at).getTime()+864e5).toISOString();await t.prepare("UPDATE scheduled_tasks SET next_run_at = ?, last_run_at = ? WHERE id = ?").bind(i,r,e.id).run()}}catch(r){console.error(`[Scheduled Task] Error processing task ${e.id}:`,r.message)}}function C(t=""){return`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
...
</html>`}function st(){return`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</html>`}var qt={async fetch(t,n,a){let e=n.DB,r=new URL(t.url),i=r.pathname,c=t.method;await D(e);let m=i.match(/^\/webhook\/(.+)$/);if(m&&c==="POST"){let u=m[1];try{let v=await e.prepare("SELECT * FROM bots WHERE bot_id = ? AND status = 'active'").bind(u).first();if(!v)return new Response(JSON.stringify({error:"Bot not found"}),{status:404,headers:{"Content-Type":"application/json"}});let _=await t.json();return a.waitUntil(at(e,v,_)),new Response("OK",{status:200})}catch(v){return console.error("[Webhook Route Error]",v.message),new Response("OK",{status:200})}}if(i==="/health")return new Response(JSON.stringify({status:"ok",time:new Date().toISOString()}),{headers:{"Content-Type":"application/json"}});if(i==="/login"&&c==="GET")return new Response(C(),{headers:{"Content-Type":"text/html; charset=utf-8"}});if(i==="/login"&&c==="POST")try{let u="";if((t.headers.get("Content-Type")||"").includes("application/json")?u=(await t.json()).password||"":u=(await t.formData()).get("password")||"",!await G(e,u))return new Response(C("\u5BC6\u7801\u9519\u8BEF\uFF0C\u8BF7\u91CD\u8BD5"),{headers:{"Content-Type":"text/html; charset=utf-8"}});let R=await H(e),w=new Headers;return w.set("Location","/"),w.set("Set-Cookie",W(R)),new Response(null,{status:302,headers:w})}catch(u){return console.error("[Login Error]",u.message),new Response(C("\u767B\u5F55\u5931\u8D25: "+u.message),{headers:{"Content-Type":"text/html; charset=utf-8"}})}if(i==="/logout"){let u=M(t);return u&&await X(e,u),new Response(null,{status:302,headers:{Location:"/login","Set-Cookie":z()}})}let b=await j(e,t);return i.startsWith("/api/")?b?nt(e,t,r):new Response(JSON.stringify({error:"\u672A\u6388\u6743\uFF0C\u8BF7\u5148\u767B\u5F55"}),{status:401,headers:{"Content-Type":"application/json"}}):i==="/"||i==="/dashboard"?b?new Response(st(),{headers:{"Content-Type":"text/html; charset=utf-8"}}):new Response(null,{status:302,headers:{Location:"/login"}}):i==="/favicon.ico"?new Response(null,{status:204}):new Response("Not Found",{status:404})},async scheduled(t,n,a){let e=n.DB;await D(e),a.waitUntil(rt(e))}};export{qt as default};
